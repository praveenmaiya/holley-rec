...
--   1. Fitment-only recommendations (no universal candidates)
--   2. min_price: 50 → 25
--   3. min recs per user: 3 (up to 4)
--   4. max_parttype_per_user: 999 → 2 (force category diversity)
--   5. Popularity-only scoring (remove intent score)
--   6. Engagement tier classification for post-hoc analysis
--
-- Purpose:
--   Improve v5.17 recommendation quality while preserving 3-tier popularity fallback.
--
...
-- Historical popularity: Extended window start for stronger signals
DECLARE pop_hist_end     DATE DEFAULT DATE '2025-08-31';
DECLARE pop_hist_start   DATE DEFAULT DATE '2024-01-01';
...
DECLARE min_required_recs INT64 DEFAULT 3;
...
-- ====================================================================================
-- STEP 1.3b: UNIVERSAL ELIGIBLE PARTS (SKIPPED in fitment-only v5.18)
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();

SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 1.3b] Universal eligible parts: skipped (fitment-only) [%d seconds]', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;
...
-- ====================================================================================
-- STEP 2: SCORING (Popularity-only with 3-tier fallback)
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();

-- -----------------------------------------------------------------------------------
-- STEP 2.1: INTENT SCORES (SKIPPED in v5.18)
-- -----------------------------------------------------------------------------------
SELECT '[Step 2.1] Intent scoring skipped (popularity-only v5.18)' AS log;

-- -----------------------------------------------------------------------------------
-- STEP 2.2: SEGMENT POPULARITY SCORES (Full order history)
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY v1_make, v1_model, sku AS
WITH
-- Segment purchases from full import_orders history
...
""", tbl_segment_popularity, tbl_import_orders_filtered, tbl_users)
...
FROM combined;
""", tbl_global_popularity, tbl_import_orders_filtered, tbl_users);
...
-- -----------------------------------------------------------------------------------
-- STEP 3.2: SCORED RECOMMENDATIONS (Fitment-only + 3-tier popularity fallback)
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY user_id AS
...
SELECT
  ...
  seg.rank_in_segment,
  mk.rank_in_make,
  -- V5.18: Popularity-only score (intent removed)
  ROUND(
    CASE
      WHEN COALESCE(st.segment_total_orders, 0) >= @min_segment_for_use
        THEN COALESCE(seg.segment_popularity_score, 0)
...
""", tbl_scored, tbl_users, tbl_eligible_parts, tbl_sku_images,
...
-- -----------------------------------------------------------------------------------
-- STEP 3.4: FITMENT-ONLY RANKING (Top 4, require minimum 3)
-- -----------------------------------------------------------------------------------
-- Rank fitment recommendations by score and keep up to 4.
-- Users with fewer than 3 recs are dropped.
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY user_id AS
...
SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 3] Recommendations (exclusion + dedup + diversity + fitment ranking + pivot): %d seconds', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;
...
-- V5.18: Check fitment-count distribution
EXECUTE IMMEDIATE FORMAT("""
