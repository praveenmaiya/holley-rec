-- ====================================================================================
-- V5.17 Top 100 Vehicle Spot Check Output
-- ------------------------------------------------------------------------------------
-- Purpose:
...
--
-- Notes:
--   - One representative v5.17 recommendation row is selected per vehicle (YMM).
...
--
-- Usage:
--   bq query --use_legacy_sql=false < sql/validation/v5_17_top_100_vehicle_spot_check.sql
-- ====================================================================================
...
SELECT
  fn.*,
  COUNT(*) OVER (PARTITION BY fn.v1_year, fn.v1_make, fn.v1_model) AS v5_17_user_count,
  ROW_NUMBER() OVER (
...
-- 1) Same-format client output in exact rank order
SELECT
  ov.rank AS rank,
  ov.v1_year AS year,
  ov.v1_make AS make,
  ov.v1_model AS model,
  vr.v5_17_user_count AS user_count,
  vr.rec_part_1 AS sample_rec1,
  CONCAT('https://www.holley.com/products/parts/', vr.rec_part_1) AS rec1_link,
  vr.rec1_price AS sample_rec1_price,
  vr.rec_part_2 AS sample_rec2,
  CONCAT('https://www.holley.com/products/parts/', vr.rec_part_2) AS rec2_link,
  vr.rec2_price AS sample_rec2_price,
  vr.rec_part_3 AS sample_rec3,
  CONCAT('https://www.holley.com/products/parts/', vr.rec_part_3) AS rec3_link,
  vr.rec3_price AS sample_rec3_price,
  vr.rec_part_4 AS sample_rec4,
  CONCAT('https://www.holley.com/products/parts/', vr.rec_part_4) AS rec4_link,
  vr.rec4_price AS sample_rec4_price
