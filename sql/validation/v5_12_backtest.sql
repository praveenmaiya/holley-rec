-- ==================================================================================================
-- V5.12 No Diversity Filter Backtest
-- --------------------------------------------------------------------------------------------------
-- Hypothesis: Forcing max 2 SKUs per PartType might hurt matches.
--             Users may want multiple items from same category.
-- ==================================================================================================
...
SELECT '=== V5.12 NO DIVERSITY FILTER BACKTEST ===' AS status;
...
-- ==================================================================================================
-- V5.12: NO DIVERSITY FILTER (same as V5.7 but without PartType limit)
-- ==================================================================================================
...
-- NO DIVERSITY FILTER - skip the PartType limit step
-- Just rank by score
ranked AS (
...
SELECT r.user_id, r.email_lower, r.v1_year, r.v1_make, r.v1_model,
  ...
  MAX(CASE WHEN rank_v512 = 1 THEN part_type END) AS rec1_part_type,
  MAX(CASE WHEN rank_v512 = 2 THEN sku END) AS rec_part_2,
  MAX(CASE WHEN rank_v512 = 2 THEN part_type END) AS rec2_part_type,
  MAX(CASE WHEN rank_v512 = 3 THEN sku END) AS rec_part_3,
  MAX(CASE WHEN rank_v512 = 3 THEN part_type END) AS rec3_part_type,
  MAX(CASE WHEN rank_v512 = 4 THEN sku END) AS rec_part_4,
  MAX(CASE WHEN rank_v512 = 4 THEN part_type END) AS rec4_part_type
...
SELECT 'V5.12 recs' AS step, COUNT(*) AS users FROM v5_12_recs_wide;

-- V5.7 BASELINE (with diversity filter)
...
SELECT 'V5.7 baseline' AS step, COUNT(*) AS users FROM v5_7_recs_wide;
...
-- RESULTS
SELECT '=== V5.12 BACKTEST RESULTS ===' AS section;
...
FROM (
  SELECT 'v5.7 (with diversity)' AS version,
  ...
  UNION ALL
  SELECT 'v5.12 (no diversity)' AS version,
...
SELECT 'V5.12 Match Slot Distribution' AS analysis, matched_slot, COUNT(*) AS match_count
FROM v5_12_matches GROUP BY matched_slot ORDER BY matched_slot;

SELECT 'SKU Diversity' AS analysis, version, COUNT(DISTINCT rec_part_1) AS unique_rec1
FROM (SELECT 'v5.7' AS version, rec_part_1 FROM v5_7_recs_wide UNION ALL SELECT 'v5.12' AS version, rec_part_1 FROM v5_12_recs_wide)
GROUP BY version;

-- Check if all 4 recs are same PartType in V5.12
SELECT 'Part type concentration' AS analysis,
  COUNTIF(rec1_part_type = rec2_part_type AND rec2_part_type = rec3_part_type AND rec3_part_type = rec4_part_type) AS all_4_same_type,
  COUNT(*) AS total_users,
  ROUND(100.0 * COUNTIF(rec1_part_type = rec2_part_type AND rec2_part_type = rec3_part_type AND rec3_part_type = rec4_part_type) / COUNT(*), 2) AS pct_all_same
FROM v5_12_recs_wide;

SELECT 'V5.12 Sample Matches' AS analysis, email_lower, v1_year, v1_make, v1_model, purchased_sku, matched_slot
FROM v5_12_matches LIMIT 20;
