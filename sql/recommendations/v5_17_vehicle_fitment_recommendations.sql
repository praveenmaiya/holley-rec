-- ==================================================================================================
-- Holley Vehicle Fitment Recommendations – V5.17 (3-Tier Segment Fallback)
-- --------------------------------------------------------------------------------------------------
-- Based on V5.16 (Segment-Based Popularity), with the following improvement:
--   1. 3-tier popularity fallback for sparse segments:
--      - Tier 1: Make/Model segment (e.g., FORD/MUSTANG) - 86.7% of users
--      - Tier 2: Make-only (e.g., FORD) - 10% of users with sparse segments
--      - Tier 3: Global - 3.3% of users with both sparse
--
-- Problem with V5.16:
--   - 13% of users have sparse make/model data (<5 orders)
--   - These users fall back to global popularity (less relevant)
--
-- V5.17 Solution:
--   - Add make-level popularity as intermediate fallback
--   - Users with sparse FORD/MUSTANG data get FORD-wide popularity
--   - More relevant than random global products
--   - Expected improvement: Better recs for 10% of users (50K users)
--
-- Fallback Logic:
--   IF segment_orders >= 5: use segment_popularity_score
--   ELIF make_orders >= 20: use make_popularity_score
--   ELSE: use global_popularity_score
--
...
-- Pipeline version
DECLARE pipeline_version STRING DEFAULT 'v5.17';

-- Working dataset (intermediate tables)
DECLARE target_project STRING DEFAULT 'auxia-reporting';
DECLARE target_dataset STRING DEFAULT 'temp_holley_v5_17';
...
DECLARE tbl_make_popularity STRING DEFAULT FORMAT('`%s.%s.make_popularity`', target_project, target_dataset);  -- V5.17 NEW
...
-- -----------------------------------------------------------------------------------
-- STEP 2.2c: MAKE-LEVEL POPULARITY (V5.17 NEW - Intermediate fallback)
-- -----------------------------------------------------------------------------------
-- Aggregate segment popularity by MAKE only (e.g., all FORD models combined)
-- Used when make/model segment is sparse but make has enough data
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
...
-- ====================================================================================
-- STEP 3: USER RECOMMENDATIONS (V5.17: 3-tier popularity fallback)
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();
...
-- -----------------------------------------------------------------------------------
-- Fallback logic:
--   IF segment_orders >= min_segment_for_use (5): use segment_popularity_score
--   ELIF make_orders >= min_make_for_use (20): use make_popularity_score
--   ELSE: use global_popularity_score
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
...
SELECT
  ...
  seg.rank_in_segment,
  mk.rank_in_make,  -- V5.17: Include make rank for debugging
...
-- ====================================================================================
-- VALIDATION: Final Output Checks (V5.17: Include 3-tier scoring distribution)
-- ====================================================================================
