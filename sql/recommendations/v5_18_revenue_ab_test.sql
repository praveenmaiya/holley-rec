-- ==================================================================================================
-- Holley Vehicle Fitment Recommendations – V5.18 (Revenue A/B Test)
-- --------------------------------------------------------------------------------------------------
-- Based on V5.17 (3-Tier Segment Fallback), with the following changes:
--   1. Reserved slot allocation: 2 fitment + 2 universal (guaranteed diversity)
--   2. max_parttype_per_user: 999 → 2 (force category diversity)
--   3. max_universal_products: 500 → 1000 (broader discovery)
--   4. Engagement tier classification for post-hoc revenue analysis
--   5. Output columns: + engagement_tier, fitment_count
--
-- Purpose:
--   One-time email blast A/B test to prove personalized recs drive revenue.
--   We control audience selection. Contract depends on it.
--
-- NOT changing (iteration history proved these hurt):
...
-- --------------------------------------------------------------------------------------------------
-- Usage:
--   bq query --use_legacy_sql=false < sql/recommendations/v5_18_revenue_ab_test.sql
-- ==================================================================================================
...
DECLARE max_parttype_per_user INT64 DEFAULT 2;  -- V5.18: Force category diversity (was 999)
DECLARE required_recs INT64 DEFAULT 4;

-- V5.18: Expanded universal pool for broader discovery (was 500)
DECLARE max_universal_products INT64 DEFAULT 1000;

-- V5.17: 3-tier popularity configuration
...
DECLARE min_segment_for_use INT64 DEFAULT 5;  -- Minimum segment orders to use segment popularity
DECLARE min_make_for_use INT64 DEFAULT 20;  -- Minimum make orders to use make popularity
DECLARE make_popularity_weight FLOAT64 DEFAULT 8.0;  -- Slightly lower than segment (10.0)
...
DECLARE tbl_audience STRING DEFAULT FORMAT('`%s.%s.audience_qualified`', target_project, target_dataset);  -- V5.18 NEW
...
DECLARE tbl_segment_popularity STRING DEFAULT FORMAT('`%s.%s.segment_popularity`', target_project, target_dataset);  -- V5.16
DECLARE tbl_make_popularity STRING DEFAULT FORMAT('`%s.%s.make_popularity`', target_project, target_dataset);  -- V5.17
DECLARE tbl_global_popularity STRING DEFAULT FORMAT('`%s.%s.global_popularity_fallback`', target_project, target_dataset);  -- V5.16
...
-- ====================================================================================
-- STEP 0.5: AUDIENCE QUALIFICATION (V5.18 NEW)
-- ====================================================================================
-- Classify users by engagement tier for post-hoc revenue analysis
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();
...
CLUSTER BY user_id AS
SELECT u.*,
  'pending' AS engagement_tier  -- Placeholder; updated after staged_events
FROM %s u;
""", tbl_audience, tbl_users);

SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 0.5] Audience qualified (placeholder): %d seconds', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;
...
-- ====================================================================================
-- STEP 0.5b: UPDATE AUDIENCE ENGAGEMENT TIERS (V5.18)
-- ====================================================================================
-- Now that staged_events exists, classify users by engagement
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();
...
CREATE OR REPLACE TABLE %s
CLUSTER BY user_id AS
SELECT u.user_id, u.email_lower, u.email_upper, u.v1_year, u.v1_year_int, u.v1_make, u.v1_model,
  CASE
    WHEN EXISTS (SELECT 1 FROM %s se WHERE se.user_id = u.user_id
                 AND UPPER(se.event_name) IN ('PLACED ORDER','ORDERED PRODUCT','CONSUMER WEBSITE ORDER','CART UPDATE'))
      THEN 'hot'
    WHEN EXISTS (SELECT 1 FROM %s se WHERE se.user_id = u.user_id
                 AND UPPER(se.event_name) = 'VIEWED PRODUCT')
      THEN 'warm'
    ELSE 'cold'
  END AS engagement_tier
FROM %s u;
""", tbl_audience, tbl_staged_events, tbl_staged_events, tbl_users);

SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 0.5b] Audience engagement tiers: %d seconds', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;

EXECUTE IMMEDIATE FORMAT("""
SELECT 'audience_engagement' AS check_name,
...
-- -----------------------------------------------------------------------------------
-- STEP 2.2c: MAKE-LEVEL POPULARITY (V5.17 - Intermediate fallback)
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY v1_make, sku AS
WITH
-- Aggregate segment orders by make (sum across all models)
make_aggregated AS (
  SELECT
    v1_make,
    sku,
...
SELECT
  ...
  sku,
  total_make_orders,
  -- V5.17: Make-level score (between segment 10.0 and global 2.0)
...
WHERE total_make_orders >= 2;  -- Minimum threshold for make-level
""", tbl_make_popularity, tbl_segment_popularity)
USING make_popularity_weight AS make_popularity_weight;

SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 2] Scoring (intent + 3-tier popularity): %d seconds', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;

-- V5.17: Report segment and make coverage
EXECUTE IMMEDIATE FORMAT("""
...
-- V5.17: Report make-level coverage
EXECUTE IMMEDIATE FORMAT("""
...
-- ====================================================================================
-- STEP 3: USER RECOMMENDATIONS (V5.18: Reserved Slot Allocation)
-- ====================================================================================
SET step_start = CURRENT_TIMESTAMP();
...
-- -----------------------------------------------------------------------------------
-- STEP 3.2: SCORED RECOMMENDATIONS (V5.17: 3-tier popularity fallback)
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY user_id AS
...
-- V5.17: Pre-compute segment totals per make/model to determine fallback tier
segment_totals AS (
  SELECT v1_make, v1_model, SUM(total_segment_orders) AS segment_total_orders
  FROM %s
  GROUP BY v1_make, v1_model
),
-- V5.17: Pre-compute make totals to determine fallback tier
make_totals AS (
...
SELECT
  ...
  -- V5.17: 3-tier popularity fallback
  CASE
    WHEN COALESCE(st.segment_total_orders, 0) >= @min_segment_for_use
      THEN COALESCE(seg.segment_popularity_score, 0)
    WHEN COALESCE(mt.make_total_orders, 0) >= @min_make_for_use
      THEN COALESCE(mk.make_popularity_score, 0)
    ELSE COALESCE(glob.global_popularity_score, 0)
  END AS popularity_score,
  -- V5.17: Track which tier was used
  CASE
    WHEN COALESCE(st.segment_total_orders, 0) >= @min_segment_for_use THEN 'segment'
    WHEN COALESCE(mt.make_total_orders, 0) >= @min_make_for_use THEN 'make'
    WHEN glob.global_popularity_score IS NOT NULL THEN 'global'
    ELSE 'none'
  END AS popularity_source,
  seg.rank_in_segment,
  mk.rank_in_make,
  -- V5.17: Final score with 3-tier fallback
  ROUND(COALESCE(int.intent_score, 0) +
    CASE
      WHEN COALESCE(st.segment_total_orders, 0) >= @min_segment_for_use
        THEN COALESCE(seg.segment_popularity_score, 0)
      WHEN COALESCE(mt.make_total_orders, 0) >= @min_make_for_use
...
-- V5.17: Join all three popularity tiers
LEFT JOIN %s seg ON ac.v1_make = seg.v1_make AND ac.v1_model = seg.v1_model AND ac.sku = seg.sku
LEFT JOIN %s mk ON ac.v1_make = mk.v1_make AND ac.sku = mk.sku
LEFT JOIN %s glob ON ac.sku = glob.sku
-- V5.17: Join tier totals for fallback logic
LEFT JOIN segment_totals st ON ac.v1_make = st.v1_make AND ac.v1_model = st.v1_model
LEFT JOIN make_totals mt ON ac.v1_make = mt.v1_make
...
""", tbl_scored, tbl_users, tbl_eligible_parts, tbl_sku_images, tbl_users, tbl_universal_parts,
    ...
    tbl_intent, tbl_segment_popularity, tbl_make_popularity, tbl_global_popularity, tbl_purchase_excl)
...
-- -----------------------------------------------------------------------------------
-- STEP 3.4: RESERVED SLOT SELECTION (V5.18: 2 fitment + 2 universal with backfill)
-- -----------------------------------------------------------------------------------
-- Instead of top 4 by score, reserve slots:
--   Slots 1-2: Top 2 fitment candidates
--   Slots 3-4: Top 2 universal candidates
--   Backfill: If a pool has <2, fill from the other pool
-- -----------------------------------------------------------------------------------
EXECUTE IMMEDIATE FORMAT("""
CREATE OR REPLACE TABLE %s
CLUSTER BY user_id AS
WITH
-- Rank fitment candidates per user
fitment_ranked AS (
  SELECT d.*,
         ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY final_score DESC, sku) AS fit_rank
  FROM %s d
  WHERE product_type = 'fitment'
),
-- Rank universal candidates per user
universal_ranked AS (
  SELECT d.*,
         ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY final_score DESC, sku) AS uni_rank
  FROM %s d
  WHERE product_type = 'universal'
),
-- Count available per pool per user
pool_counts AS (
  SELECT
    user_id,
    COUNTIF(product_type = 'fitment') AS fitment_available,
    COUNTIF(product_type = 'universal') AS universal_available
  FROM %s
  GROUP BY user_id
),
-- Take top 2 fitment (or fewer if not enough)
fitment_selected AS (
  SELECT f.*, 'fitment_slot' AS slot_source
  FROM fitment_ranked f
  WHERE f.fit_rank <= 2
),
-- Take top 2 universal (or fewer if not enough)
universal_selected AS (
  SELECT u.*, 'universal_slot' AS slot_source
  FROM universal_ranked u
  WHERE u.uni_rank <= 2
),
-- Backfill: if fitment has <2, take extra universal
universal_backfill AS (
  SELECT u.*, 'backfill_for_fitment' AS slot_source
  FROM universal_ranked u
  JOIN pool_counts pc ON u.user_id = pc.user_id
  WHERE pc.fitment_available < 2
    AND u.uni_rank > 2
    AND u.uni_rank <= 2 + (2 - pc.fitment_available)
),
-- Backfill: if universal has <2, take extra fitment
fitment_backfill AS (
  SELECT f.*, 'backfill_for_universal' AS slot_source
  FROM fitment_ranked f
  JOIN pool_counts pc ON f.user_id = pc.user_id
  WHERE pc.universal_available < 2
    AND f.fit_rank > 2
    AND f.fit_rank <= 2 + (2 - pc.universal_available)
),
-- Combine all selected recs
combined AS (
  SELECT *, 1 AS sort_group FROM fitment_selected
  UNION ALL
  SELECT *, 1 AS sort_group FROM fitment_backfill
  UNION ALL
  SELECT *, 2 AS sort_group FROM universal_selected
  UNION ALL
  SELECT *, 2 AS sort_group FROM universal_backfill
),
-- Final ranking: fitment first, then universal, ordered by score within each group
final_ranked AS (
  SELECT c.*,
         ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY sort_group, final_score DESC, sku) AS rn,
...
""", tbl_ranked, tbl_diversity, tbl_diversity, tbl_diversity)
...
WITH rec_data AS (
  SELECT r.*,
    aq.engagement_tier
  FROM %s r
  JOIN %s aq ON r.user_id = aq.user_id
)
SELECT
  email_lower,
  v1_year,
  ...
  -- V5.18: New columns for revenue analysis
  MAX(engagement_tier) AS engagement_tier,
  COUNTIF(product_type = 'fitment') AS fitment_count,
  CURRENT_TIMESTAMP() AS generated_at,
  @pipeline_version AS pipeline_version
FROM rec_data
GROUP BY email_lower, v1_year, v1_make, v1_model
HAVING COUNT(*) = 4;
""", tbl_final, tbl_ranked, tbl_audience)
USING pipeline_version AS pipeline_version;

SET step_end = CURRENT_TIMESTAMP();
SELECT FORMAT('[Step 3] Recommendations (exclusion + dedup + diversity + reserved slots + pivot): %d seconds', TIMESTAMP_DIFF(step_end, step_start, SECOND)) AS log;

-- ====================================================================================
-- VALIDATION: Final Output Checks (V5.18: Include reserved slot + engagement checks)
-- ====================================================================================
...
-- V5.18: Check reserved slot distribution
EXECUTE IMMEDIATE FORMAT("""
SELECT 'reserved_slot_distribution' AS check_name,
...
-- V5.18: Check engagement tier distribution
EXECUTE IMMEDIATE FORMAT("""
...
-- V5.17: Check popularity source distribution (3-tier: segment vs make vs global)
EXECUTE IMMEDIATE FORMAT("""
...
-- V5.18: Category diversity check (max 2 per PartType enforced)
EXECUTE IMMEDIATE FORMAT("""
SELECT 'category_diversity' AS check_name,
  COUNT(DISTINCT part_type) AS unique_part_types,
  CASE WHEN COUNT(DISTINCT part_type) >= 400 THEN 'OK'
       ELSE FORMAT('WARNING: Only %%d unique PartTypes (target >=400)', COUNT(DISTINCT part_type))
  END AS status
FROM (
  SELECT rec_part_1 AS sku FROM %s
  UNION DISTINCT
  SELECT rec_part_2 FROM %s
  UNION DISTINCT
  SELECT rec_part_3 FROM %s
  UNION DISTINCT
  SELECT rec_part_4 FROM %s
) all_skus
LEFT JOIN `auxia-gcp.data_company_1950.import_items` cat
  ON all_skus.sku = UPPER(TRIM(cat.PartNumber))
""", tbl_final, tbl_final, tbl_final, tbl_final);
